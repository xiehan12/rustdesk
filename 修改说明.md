...
4. **静默安装自定义路径** - 支持 `--silent-install --path="路径"` 指定安装目录
...
| 文件 | 修改内容 |
|------|----------|
| `src/core_main.rs` | 添加 `--path=<路径>` 参数解析 |

#### 修改详情

**文件：** `src/core_main.rs` 第 296-313 行
...
```powershell
# 默认路径安装（C:\Program Files\RustDesk）
...
完整的静默安装说明请参考：**[静默安装说明.md](静默安装说明.md)**
